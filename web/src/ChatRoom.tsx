...
export function ChatRoom(props: { roomId: string; onLeave: () => void }) {
  const { roomId } = props;
  ...
  // Remember the last room visited so the Landing page can offer a "return" link.
  useEffect(() => {
    try {
  ...
  // Reset all room-scoped state when the room id changes.
  useEffect(() => {
    setMessages([]);
    setOnline([]);
    setTypingUsers({});
          ...
          setMessages((prev) => {
            if (ev.clientId) {
              const idx = prev.findIndex(
                (m) => m.clientId === ev.clientId && m.pending,
              );
              if (idx >= 0) {
  ...
  const loadSince = useCallback(async () => {
    try {
      if (lastSeenIdRef.current === 0) {
        const recent = await fetchMessages({ room: roomId, limit: 50 });
        mergeMessages(recent);
      } else {
        const missed = await fetchMessages({
          room: roomId,
          since: lastSeenIdRef.current,
        });
  ...
  // Username picker is shown as an overlay so the room context remains visible.
  if (!user) {
    return (
      <div className="flex flex-col h-full max-w-3xl mx-auto bg-ink-900">
        ...
        <div className="flex-1 relative">
          <div className="absolute inset-0 bg-ink-900/80 backdrop-blur-sm grid place-items-center p-6">
            <UsernameGate onSubmit={handleUsername} roomId={roomId} />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full max-w-3xl mx-auto bg-ink-900">
      <PresenceBar
        ...
        onLeave={props.onLeave}
      />
      {status === "open" && online.length <= 1 && (
        <EmptyRoomBanner roomId={roomId} />
      )}
